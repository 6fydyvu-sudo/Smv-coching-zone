...
export default async function StudentDashboardPage() {
  ...
  const attendancePercent =
    totalClasses > 0 ? Math.round((presentCount / totalClasses) * 100) : null;
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <p className="text-lg font-semibold text-slate-900">
            {student.course?.nameEn ?? "Not assigned"}
          </p>
        </Card>
          ...
          {latestResult ? (
            <div className="text-sm">
              <p className="font-medium text-slate-900">
                {latestResult.exam.titleEn}
              </p>
              <p className="text-slate-500">
                {latestResult.exam.subject.nameEn}
              </p>
              <p className="mt-1">
                {latestResult.isAbsent
                  ? "Absent"
                  : `${latestResult.marksObtained} / ${latestResult.exam.fullMarks}`}
                {latestResult.grade && (
                  <Badge tone="blue">{latestResult.grade}</Badge>
                )}
              </p>
            </div>
          ) : (
